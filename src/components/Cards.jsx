       ...
       {matchingPosts.map((post) => (
            ...
            <div className="container max-w-5xl py-6 rounded-lg shadow-sm dark:bg-gray-900">
              <div className="flex items-center justify-between">
                <span className="text-sm dark:text-gray-400">{new Date(post.date_added).toLocaleDateString('en-US', {year: 'numeric',month: 'short',day: 'numeric'})}</span>
                <a rel="noopener noreferrer" href="#" className="py-1 font-bold rounded dark:bg-violet-400 dark:text-gray-900">{post.title}</a>
              </div>
              <div className="mt-3">
                <a rel="noopener noreferrer" href="#" className="text-2xl font-bold hover:underline">{post.description}</a>
                <p className="mt-2 ">{post.content.length > 100 ? post.content.substring(0,100):post.content } .....</p>
              </div>
              <div className="flex items-center justify-between mt-4">
                      <Link to = {`/blog/${post.id}`} class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-2 text-center inline-flex items-center  dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                          Read more 
                ...
                <div>
                  <a rel="noopener noreferrer" href="#" className="flex items-center">
                    ...
                    <span className="hover:underline dark:text-gray-400">{}</span>
                  </a>
                </div>
              </div>
                  </div>
